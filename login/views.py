...
def home(request):
    ...
    return render(request, 'home.html',{'cancount':cancount,'comcount':comcount,'jobcount':jobcount,'slt':slt})
...
def about(request):
    ...
    return render(request, 'about.html',{'cancount':cancount,'comcount':comcount,'jobcount':jobcount,'slt':slt,'app':app,'shrt':shrt})
    ...
    if request.method == 'POST':
        ...
        if(password1 == password2):
            if User.objects.filter(username=username).exists():
                ...
                user.save()
                com = com_pro(com_username=username, com_name =first_name, com_desc=desc, com_place=place, com_pincode=pincode, com_dt=dt, com_state=state, com_country = country, com_mob =mob, com_email =email,status=1,logo="")
                com.save()
