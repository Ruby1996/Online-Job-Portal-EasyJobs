...
def can_about(request):
    ...
    if request.method == 'POST': 
        ...
        if srch != "" and loc != "":
            ...
            return render(request,'can_about.html',{'cancount':cancount,'comcount':comcount,'jobcount':jobcount,'slt':slt,'app':app,'sht':shrt})          

    return render(request, 'can_about.html',{'cancount':cancount,'comcount':comcount,'jobcount':jobcount,'slt':slt,'app':app,'shrt':shrt})    
...
def can_jobs(request, uname):
    ...
    if canpro.school != "" and canpro.sc_board != "" and canpro.sc_percent != "" and canpro.ug_course == "" and canpro.pg_course == "" and canpro.hss_stream == "":
        jobs = Jobpost.objects.filter(last_date__gte= date.today(), job_qua__in=["10","sslc","tenth","10th"],status=1).order_by('-post_date')
        comp =[]
        logo=[]
        for e in jobs:
            c = e.com_username
        ...
        jobs = Jobpost.objects.filter(Q(job_qua__icontains=canpro.ug_course) | Q(job_qua__icontains= canpro.hss_stream)| Q(job_qua__iexact="12") ,last_date__gte= date.today(),status=1).order_by('-post_date')
        ...
        jobs = Jobpost.objects.filter( Q(job_qua__icontains= canpro.pg_course) | Q(job_qua__icontains=canpro.ug_course) | Q(job_qua__icontains= canpro.hss_stream)| Q(job_qua__iexact="12") ,last_date__gte= date.today(),status=1).order_by('-post_date')
...
def can_noti(request, uname):
    ...
    for e in intrv:
        j =Jobpost.objects.get(id=e.job_id,status=1)
        n = j.job_name
        cuname = j.com_username
        co = com_pro.objects.get(com_username=cuname)
        cname = co.com_name
        com.append(cname)
        result.append(n)
        c = c+1
    ...
    for e in admn:
        ...
        # cuname = hlp.username
        # c = can_pro.objects.filter(can_uname = cuname).count()
    ...
    return render(request, 'can_noti.html', {'intrv':intrv,'result':result,'com':com,'rply':rply,'date':date,'rid':rid})   
...
def can_app(request, uname):
    ...
    for e in jobs:
        ...
        comname = comp.com_name
        com.append(comname)
        jb = Jobpost.objects.get(id=jid,status=1)
        jbname = jb.job_name
        ap.append(jbname)
    ...
    return render(request, 'can_app.html',{'jobs':jobs,'ap':ap,'com':com})     
     ...
     #   messages.info(request,'Not Eligible for the post')    
       # return redirect('can_jobs')
        ...
        if canpro.school != "" and canpro.sc_board != "" and canpro.sc_percent != "" and canpro.ug_course == "" and canpro.pg_course == "" and canpro.hss_stream == "":
            ...
            flag = 3
            jobs = Jobpost.objects.filter( Q(job_qua__icontains= canpro.pg_course) | Q(job_qua__icontains=canpro.ug_course) | Q(job_qua__icontains= canpro.hss_stream)| Q(job_qua__iexact="12")| Q(job_qua__icontains="any degree") ,last_date__gte= date.today(),status=1)
...
def can_saveprofile(request, uname):
    ...
    if request.method == 'POST':
        ...
        canpro = can_pro.objects.get(can_uname = uname,status=1)
        res = request.FILES.get('resume',False)     
        if res == False:
            pass
        else:
...
def can_comhome(request,uname,canuname):
    ...
    if canpro.school != "" and canpro.sc_board != "" and canpro.sc_percent != "" and canpro.ug_course == "" and canpro.pg_course == "" and canpro.hss_stream == "":
        jobs = Jobpost.objects.filter(last_date__gte= date.today(), job_qua__in=["10","sslc","tenth","10th"],com_username=uname,status=1)
        return render(request, 'can_jobs.html', {'jobs':jobs})

    elif canpro.school != "" and canpro.sc_board != "" and canpro.sc_percent != "" and canpro.hss_stream != "" and canpro.ug_course == "" and canpro.pg_course == "":
        jobs = Jobpost.objects.filter( Q(job_qua__exact= "12") | Q(job_qua__exact= "twelveth") | Q(job_qua__exact= "twelve")| Q(job_qua__icontains= "10") |  Q(job_qua__icontains= "sslc") | Q(job_qua__icontains= "tenth") |  Q(job_qua__icontains= "10th") |  Q(job_qua__icontains= "ten") |   Q(job_qua__icontains= canpro.hss_stream)  , last_date__gte= date.today(),com_username=uname,status=1)
        return render(request, 'can_jobs.html', {'jobs':jobs})
 
    else:    
    
        jobs = Jobpost.objects.filter( Q(job_qua__icontains= canpro.pg_course) | Q(job_qua__icontains=canpro.ug_course) | Q(job_qua__icontains= canpro.hss_stream)| Q(job_qua__iexact="12") ,last_date__gte= date.today(),com_username=uname,status=1)
...
def can_img(request):
    return render(request,'can_img.html')

def can_saveimg(request,uname):
    can = can_pro.objects.get(can_uname=uname)
    if request.method == 'POST':    
...
def can_help(request):
    return render(request,'can_help.html')

def can_help_submit(request,uname):
    if request.method == 'POST':
...
def can_reject_reply(request,uname,id):
